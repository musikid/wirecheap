...
public class ParserTest {
    ...
    @Test
    @DisplayName("fragments combinator")
    void testFragments() throws ParseException {
        List<Fragment> expected = Arrays.asList(
                new Fragment(
                        0,
                        Stream.of(
                                0x54, 0x8d, 0x5a, 0x56, 0xc7, 0x55, 0xbe, 0x9a, 0xe5, 0xd5, 0x31, 0x7d,
                                0x08, 0x00, 0x45, 0x28
                ...
                new Fragment(
                        0x30,
                        Stream.of(0x00, 0x00, 0x28, 0x24, 0x00, 0x00
                        ).map(Integer::byteValue).collect(Collectors.toList())
                ...
                new Fragment(
                        0x10,
                        Stream.of(
                                0x00, 0x28, 0x27, 0x1b, 0x00, 0x00, 0x80, 0x06, 0x00, 0x00, 0xc0, 0xa8,
                                0x2b, 0xad, 0xa2, 0x9f
                        ).map(Integer::byteValue).collect(Collectors.toList())),
                ...
                new Fragment(
                        0x20,
                        Stream.of(
                                0x86, 0xea, 0xe6, 0x63, 0x01, 0xbb, 0xed, 0x29, 0x53, 0xc0, 0x32, 0x0a,
                                0xae, 0x3d, 0x50, 0x10
                        ).map(Integer::byteValue).collect(Collectors.toList())),
                new Fragment(
                        0x30,
                        Stream.of(0x00, 0xff, 0x8f, 0xa5, 0x00, 0x00)
                                .map(Integer::byteValue).collect(Collectors.toList()))
        );
        ...
        List<Fragment> actual = parser.parse(testCase);
